...
for chromosome_num in tqdm(range(21,45), desc="Processing Chromosomes", unit="chromosome", colour="blue"):
    ...
    input_baypass_path = rf'Sorted_Results/Sparus_{chromosome}_baypass_summary_pi_xtx.out'
    input_maf_path = rf'maf_{chromosome}.csv'
    output_path = f'final_output_after_sort/Sparus_{chromosome}_baypass_result.csv'
...
for i in range(21, 44 + 1):
    # Format the filename
    filename = rf"final_output_after_sort/Sparus_LR5371{i}_baypass_result.csv"
    # Read the CSV file
...
merged_df.to_csv("final_output_after_sort/merged_baypass_results.csv", index=False)
