...
def optimized_remove_rows(genotype_file, baypass_file, max_threshold):
    ...
    print(f"Threshold 54: Removed {removed_rows_count} rows cumulatively.")
    ...
    snp_df.to_csv(f'processed_files/{genotype_file}', sep=" ", header=None, index=False)
    print(f"Modified data written back to {genotype_file}")
    baypass_df.to_csv(f'processed_files/{baypass_file}', sep=" ", header=None, index=False)
    print(f"Modified data written back to {baypass_file}")
    #---------------------------------------------------------------------------------------------
...
for genotype_file, baypass_file in zip(genotype_files,baypass_files):
    print(optimized_remove_rows(genotype_file, baypass_file, 54))
